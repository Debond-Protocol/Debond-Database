...
contract ERC3475 is IERC3475, IERC3475EXTENSION {
    ...
    returns (uint256 progressAchieved, uint256 progressRemaining){
        ...
        if (nonceId == 0){
        issuanceDate = _classes[classId]._values["issueDate"].uintValue;
        }
...
contract Conter is ERC3475{

    constructor(address _publisher) {
        publisher = _publisher;

    }
    event Subscribe(address indexed _to, address instrument, uint256 _classId, address token, uint256 _amount, string orderId);

    function checkAllowedToken(address token, address instrument) public view returns(bool){
        address[] memory allowed = IERC3475EXTENSION(instrument).classValuesFromTitle(0, "collateralAllowed").addressArrayValue;
     ...
     function subscribe(
        ...
        require (checkAllowedToken(token,instrument) == true, "Token not allowed");

        IERC20(token).transferFrom(_to, publisher, amount);
        emit Subscribe(_to, instrument, class, token, amount, orderId);
    }
